...
'use strict';
{
    const DateTimeShortcuts = {
        ...
        init: function() {
            ...
            for (const inp of document.getElementsByTagName('input')) {
                if (inp.type === 'text' && inp.classList.contains('vTimeField')) {
                    DateTimeShortcuts.addClock(inp, DateTimeShortcuts.clockHours, get_format('TIME_INPUT_FORMATS')[0]);
                ...
                else if (inp.type === 'text' && inp.classList.contains('vDurationField')) {
                    DateTimeShortcuts.addClock(inp, DateTimeShortcuts.clockDurations, inp.getAttribute('data-format'));
                    DateTimeShortcuts.addTimezoneWarning(inp);
                }
            }
        },
        ...
        addClock: function(inp, hrs, format) {
            ...
            //     e.preventDefault();
            //     DateTimeShortcuts.handleClockQuicklink(num, -1, format);
            // });
            ...
            const name = typeof hrs[inp.name] === 'undefined' ? 'default_' : inp.name;
            hrs[name].forEach(function(element) {
                const time_link = quickElement('a', quickElement('li', time_list), gettext(element[0]), 'href', '#');
                time_link.addEventListener('click', function(e) {
                    e.preventDefault();
                    DateTimeShortcuts.handleClockQuicklink(num, element[1], format);
                });
            });
        ...
        handleClockQuicklink: function(num, val, format) {
            let d;
            if (val === -1) {
                d = DateTimeShortcuts.now();
            }
            else {
                d = new Date(1970, 1, 1, 0, val, 0, 0);
            }
            DateTimeShortcuts.clockInputs[num].value = d.strftime(format);
