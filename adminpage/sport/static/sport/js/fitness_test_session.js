...
fetch("/api/fitnesstest/exercises", {
  ...
  .then((data) => {
    data.forEach((ex, index) => {
      ...
      exercises.push({ex_name: ex.name, ex_unit: ex.unit, ex_select: ex.select});
    });
  })
  .catch(function (err) {
    console.log(err);
...
function save_table() {
    ...
    for(let i = 0; i < exercises.length; i++) {
        ...
        student_ids.forEach((sid) => {
            let val;
            let inp_field = document.getElementById(`student_${sid}_${i}`).getElementsByTagName('input')[0];
            let sel_field = document.getElementById(`student_${sid}_${i}`).getElementsByTagName('select')[0];
            if (inp_field){
                val = inp_field.value;
            }
            else {
            ...
            if (val === "") {
                ...
                toastr.error(`Values should be <b>positive</b> or you have not selected the option. Check <b>${ex_name} exercise</b>`, "Value error");
                cant_submit = true;
            }
